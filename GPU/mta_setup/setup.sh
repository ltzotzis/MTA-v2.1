...
./configure --prefix="${root_dir}/petsc_install" --download-fblaslapack=1 --with-cuda --with-cuda-arch=80 --download-hypre --download-hypre-configure-arguments=--enable-unified-memory --with-debugging=no -COPTFLAGS="-O3" -CXXOPTFLAGS="-O3" -FOPTFLAGS="-O3" 
make -j8 PETSC_DIR="${root_dir}/petsc" PETSC_ARCH=arch-linux-c-opt all
make -j8 PETSC_DIR="${root_dir}/petsc" PETSC_ARCH=arch-linux-c-opt install
