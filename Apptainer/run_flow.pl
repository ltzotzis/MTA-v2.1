...
####Set directory for tools
my $paraview_root = "~/applications/ParaView-4.4.0-Qt4-Linux-64bit/bin"; # GMSH directory, need to change
my $gmsh_root = "~/applications/gmsh-2.11.0-Linux/bin"; # Paraview directory, need to change
my $mpi = ""; # MPIRUN directory. if it was exported to PATH, leave it here. Otherwise, specified it like "home/OOO/ZZZ"
my $mta_root = "apptainer exec mta2.sif "; # directory of mta binary directory, default to /bin under root of MTA, recommend to change it to full path
####Set directory for tools

####Default Output
my $output_folder = "Output"; #default to Output, you can also change it through command line
####Default Output
...
####RunStage
my $run_mode = 0; #default from XML
...
####Tool Name
my $heat_solver_name = "heat"; #preset transient analysis
my $mesh_generator_name = "msh_gen"; #mesh generator
my $package_generator_name = "pkxml"; #package and die merger
my $die_generator_name = "diexml"; #DEF-LEF XMl converter
####Tool Name
...
for (my $i = 0; $i < ($#ARGV + 1); $i++)
{
	...
	{
		$run_mode = 0;
		print "RUN MTA: IC(.xml) -> MESH(.msh) -> MTA\n";
	}
	elsif ($ARGV[$i] eq "-runMESH")
	{
		$run_mode = 1;
		print "RUN MTA: MESH(.msh) -> MTA\n";
	}
	...
	elsif ($ARGV[$i] eq "-hostfile")
	{
		$i++;
		if ($ARGV[$i] ne undef)
		{
			$hostfile_name = $ARGV[$i];
			print "READIN HOSTFILE: $ARGV[$i]\n";
		}
		else
		{
			die "ERROR: no hostfile.\n";
		}
	}
...
if ($mpi_core_number > 0)
{
	...
	else
	{
		if ($mpi ne undef)
		{
			$mpi = $mpi."/"."mpirun -np ".$mpi_core_number."  -launcher fork";
		}
		else
		{
			$mpi = "mpirun -np ".$mpi_core_number." -launcher fork";
		}
	}
	if ($hostfile_name ne "")
	{
		$mpi = $mpi." -hostfile ".$hostfile_name;
	}
}
####MPI SETUP
...
if ($run_mode == 0)
{
	...
	if (-e "$xml_file_name")
	{
		if (-e "$ptrace_file_name")
		{
			...
			print "$mta_root $mpi $heat_solver_name -xml $xml_file_name -mesh $com_msh_file_name -ptrace $ptrace_file_name $thermal_configuration\n";
			system("$mta_root $mpi $heat_solver_name -xml $xml_file_name -mesh $com_msh_file_name -ptrace $ptrace_file_name $thermal_configuration");
			if ($paraview_gui == 1)
			{
	...
	if (-e "$xml_file_name")
	{
		if (-e "$ptrace_file_name")
		{
			if (-e "$msh_file_name")
			{
				...
				print "Thermal Simulation: ";
				print "$mta_root $mpi $heat_solver_name -xml $xml_file_name -mesh $msh_file_name -ptrace $ptrace_file_name $thermal_configuration\n";
				system("$mta_root $mpi $heat_solver_name -xml $xml_file_name -mesh $msh_file_name -ptrace $ptrace_file_name $thermal_configuration");
				if ($paraview_gui == 1)
				{
					system("$paraview_root/paraview $output_folder/solution.pvd &");
				}
...
elsif ($run_mode == 2)
{
	...
	if (-e "$xml_file_name")
	{
		if (-e "$ptrace_file_name")
		{
			...
			print "Thermal Simulation: ";
			print "$mta_root $mpi $heat_solver_name -xml $com_xml_file_name -mesh $com_msh_file_name -ptrace $ptrace_file_name $thermal_configuration\n";
			system("$mta_root $mpi $heat_solver_name -xml $com_xml_file_name -mesh $com_msh_file_name -ptrace $ptrace_file_name $thermal_configuration");
		}
		else
		{
			die "ERROR: CANNOT OPEN $ptrace_file_name, no file\n";
	...
	if (-e "$xml_file_name")
	{
		if (-e "$ptrace_file_name")
		{
			if (-e "$avg_file_name")
			{
				...
				print "Thermal Simulation: ";
				print "$mta_root $mpi $heat_solver_name -xml $com_xml_file_name -mesh $com_msh_file_name -ptrace $ptrace_file_name -cellopt $com_opt_file_name $thermal_configuration\n";
				system("$mta_root $mpi $heat_solver_name -xml $com_xml_file_name -mesh $com_msh_file_name -ptrace $ptrace_file_name -cellopt $com_opt_file_name $thermal_configuration");
			}
			else
			{
